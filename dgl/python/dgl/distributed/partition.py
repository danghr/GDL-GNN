...
RESERVED_FIELD_DTYPE = {
    ...
    "non_outermost_node": F.uint8,  # A flag indicates whether the node is in a partition, including HALO nodes that are not the outermost ones.
...
def _get_non_outermost_node_mask(graph, ntype_id):
    if NTYPE in graph.ndata:
        dtype = F.dtype(graph.ndata["non_outermost_node"])
        return (
            graph.ndata["non_outermost_node"]
            * F.astype(graph.ndata[NTYPE] == ntype_id, dtype)
            == 1
        )
    else:
        return graph.ndata["non_outermost_node"] == 1
...
def load_partition(part_config, part_id, load_feats=True):
    ...
    gpb, graph_name, ntypes, etypes, num_classes = load_partition_book(part_config, part_id)
...
def load_partition_book(part_config, part_id):
    ...
    assert "num_classes" in part_metadata, "cannot get the number of classes"
    ...
    num_classes = part_metadata["num_classes"]
...
def partition_graph(
    ...
    * ``balance_edges`` balances the number of edges in each partition.
    ...
    if num_parts == 1:
        ...
        parts[0].ndata["non_outermost_node"] = F.ones(
            (sim_g.num_nodes(),),
            RESERVED_FIELD_DTYPE["non_outermost_node"],
            F.cpu(),
        )
    ...
    for part_id in range(num_parts):
        ...
        if num_parts > 1:
            for ntype in g.ntypes:
                ...
                non_outermost_node_mask = _get_non_outermost_node_mask(part, ntype_id)
                # This is global node IDs.
                local_nodes = F.boolean_mask(
                    part.ndata[ndata_name], inner_node_mask
                )
                non_outermost_node = F.boolean_mask(
                    part.ndata[ndata_name], non_outermost_node_mask
                )
                if len(g.ntypes) > 1:
                    # If the input is a heterogeneous graph.
                    local_nodes = F.gather_row(sim_g.ndata[NID], local_nodes)
                    print(
                        "part {} has {} nodes of type {} and {} are inside the partition with {} not the outermost nodes".format(
                            part_id,
                            F.as_scalar(
                            ...
                            ntype,
                            len(local_nodes),
                            len(non_outermost_node),
                        )
                    )
                else:
                    print(
                        "part {} has {} nodes and {} are inside the partition with {} not the outermost nodes".format(
                            part_id, part.num_nodes(), len(local_nodes), len(non_outermost_node)
                        )
                    )

                for name in g.nodes[ntype].data:
                    if name in [NID, "inner_node", "non_outermost_node"]:
                        continue
                    node_feats[ntype + "/" + name] = F.gather_row(
                        g.nodes[ntype].data[name], part.ndata[ndata_name]
                    )
                
                node_feats[ntype + "/" + "local_node_mask"] = inner_node_mask
                node_feats[ntype + "/" + "non_outermost_node_mask"] = non_outermost_node_mask
            ...
            for etype in g.canonical_etypes:
                ...
                for name in g.edges[etype].data:
                    ...
                    ] = F.gather_row(g.edges[etype].data[name], part.edata[edata_name])
        else:
            for ntype in g.ntypes:
                if len(g.ntypes) > 1:
                    ndata_name = "orig_id"
                    ...
                    local_nodes = F.gather_row(sim_g.ndata[NID], local_nodes)
                    non_outermost_node = F.gather_row(sim_g.ndata[NID], non_outermost_node)
                else:
                    local_nodes = sim_g.ndata[NID]
                    non_outermost_node = sim_g.ndata[NID]
                for name in g.nodes[ntype].data:
                    if name in [NID, "inner_node", "non_outermost_node"]:
                        continue
                    node_feats[ntype + "/" + name] = F.gather_row(
