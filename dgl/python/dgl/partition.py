...
def _get_halo_heterosubgraph_non_outermost_node(halo_subg):
    return _CAPI_GetHaloSubgraphNonOutermostNodes_Hetero(halo_subg)
...
def partition_graph_with_halo(g, node_part, extra_cached_hops, reshuffle=False):
    ...
    for i, subg in enumerate(subgs):
        ...
        non_outermost_node = _get_halo_heterosubgraph_non_outermost_node(subg)
        inner_node = F.zerocopy_from_dlpack(inner_node.to_dlpack())
        non_outermost_node = F.zerocopy_from_dlpack(non_outermost_node.to_dlpack())
        subg = create_subgraph(
            subg, subg.induced_nodes, subg.induced_edges, inner_node
        )
        subg.ndata["inner_node"] = inner_node
        subg.ndata["non_outermost_node"] = non_outermost_node
