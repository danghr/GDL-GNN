...
class HaloHeteroSubgraph : public HeteroSubgraph {
 public:
  ...
  std::vector<IdArray> non_outermost_nodes;
};
...
HaloHeteroSubgraph GetSubgraphWithHalo(
  ...
  auto non_outermost_nodes_flag = all_nodes;
  ...
  for (int64_t i = 0; i < num_edges; i++) {
    ...
    if (it == all_nodes.end() && num_hops > 0) {
      ...
      non_outermost_nodes_flag[src_data[i]] = num_hops > 1 ? true : false;
  ...
  for (int k = 1; k < num_hops; k++) {
    ...
    for (int64_t i = 0; i < num_edges; i++) {
      ...
      if (it == all_nodes.end()) {
        all_nodes[src_data[i]] = false;
        non_outermost_nodes_flag[src_data[i]] = k < num_hops - 1 ? true : false;
        old_node_ids.push_back(src_data[i]);
  ...
  if (num_hops > 0) {
    ...
    for (int64_t i = 0; i < num_edges; i++) {
      ...
      if (it == all_nodes.end()) {
        ...
        non_outermost_nodes_flag[dst_data[i]] = false;
  ...
  std::vector<int> non_outermost_nodes(old_node_ids.size());
  for (size_t i = 0; i < old_node_ids.size(); i++) {
    dgl_id_t old_nid = old_node_ids[i];
    inner_nodes[i] = all_nodes[old_nid];
    non_outermost_nodes[i] = non_outermost_nodes_flag[old_nid];
  }
  aten::COOMatrix coo(
  ...
  halo_subg.non_outermost_nodes = {aten::VecToIdArray<int>(non_outermost_nodes, 32)};
  return halo_subg;
}
...
DGL_REGISTER_GLOBAL("partition._CAPI_DGLPartitionWithHalo_Hetero")
    .set_body([](DGLArgs args, DGLRetValue *rv) {
      std::cout << "[NOTICE] You are using the modified version of _CAPI_DGLPartitionWithHalo_Hetero" << std::endl;
      std::cout << "[NOTICE] Compiled on " << __DATE__ << " " << __TIME__ << std::endl;
      // Args: graph, node_parts, num_extra_cached_hops
      HeteroGraphRef g = args[0];
      auto hgptr = std::dynamic_pointer_cast<HeteroGraph>(g.sptr());
      CHECK(hgptr) << "Invalid HeteroGraph object";
      CHECK_EQ(hgptr->relation_graphs().size(), 1)
      ...
      // Note that node_parts is an array whose number of items is the same as nodes
      // and the value of each item is the partition ID of the corresponding node.
      ...
      for (int64_t i = 0; i < num_nodes; i++) {
        ...
        // Find (or create) an instance of mapping of part_id and vector of nodes
        // in the part
        ...
        if (it == part_map.end()) {
          ...
          // Note that it is the iterator of an unordered_map
          // it->first is the key (part_id), and it->second is the value (vector)
...
DGL_REGISTER_GLOBAL("partition._CAPI_GetHaloSubgraphNonOutermostNodes_Hetero")
    .set_body([](DGLArgs args, DGLRetValue *rv) {
      HeteroSubgraphRef g = args[0];
      auto gptr = std::dynamic_pointer_cast<HaloHeteroSubgraph>(g.sptr());
      CHECK(gptr) << "The input graph has to be HaloHeteroSubgraph";
      *rv = gptr->non_outermost_nodes[0];
    });
